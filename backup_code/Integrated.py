...
# Parameters
symbols = ['TSLA', 'AAPL', 'NVDA']  # Add more symbols if needed
...
for symbol in symbols:
    ...
    portfolio_returns.append(data['strategy_return'])

    # Add trade log to the overall portfolio trade log
    all_trade_logs.append(pd.DataFrame(trade_log))
    ...
    print(f"Annualized Return for {symbol}: {annualized_return:.2%}")
    print(f"Max Drawdown for {symbol}: {max_drawdown:.2%}")
    print(f"Sharpe Ratio for {symbol}: {sharpe_ratio:.2f}")
    print("Trade Log for ", symbol)
    print(results[symbol]['Trade Log'])

# Combine all symbols' strategy returns for portfolio-level tracking
portfolio_returns = pd.concat(portfolio_returns, axis=1).mean(axis=1)

# Portfolio-level performance metrics
portfolio_total_return = portfolio_returns.add(1).prod() - 1
portfolio_annualized_return = (1 + portfolio_total_return) ** (252 / len(portfolio_returns)) - 1
portfolio_max_drawdown = (portfolio_returns.cumsum() - portfolio_returns.cumsum().cummax()).min()
...
plt.figure(figsize=(14, 6))
plt.plot(portfolio_returns.cumsum(), label='Portfolio Cumulative Return', color='purple')
...
# Combine all trade logs from symbols into one portfolio log
portfolio_trade_log = pd.concat(all_trade_logs, axis=0)

# Save Portfolio Trade Log to CSV
portfolio_trade_log.to_csv('portfolio_trade_log.csv', index=False)

print("\nPortfolio Trade Log saved to 'portfolio_trade_log.csv'.")
