...
# === INIT ===
data_dict, trade_log, highest_price, symbol_results = {}, [], {}, {}
capital_per_symbol, position, entry_price = {}, {}, {}
...
for s in symbols:
    capital_per_symbol[s] = initial_capital * alloc[s]
    position[s], entry_price[s], highest_price[s] = 0, 0, 0
...
for date in dates:
    ...
    for symbol in symbols:
        ...
        if position[symbol] == 0 and row['Close'] > row['20d_high']:
            ...
            print(f"BUY ALERT: {symbol} broke out on {date.date()} at ${entry_price[symbol]:.2f}")
            if SOUND_ENABLED:
                winsound.Beep(880, 300)

            trade_log.append({
                'symbol': symbol, 'date': date, 'action': 'BUY',
                'price': entry_price[symbol], 'capital_allocated': cap
            })
        ...
        # === Exit using trailing stop ===
            ...
            if row['Low'] <= trailing_stop:
                ...
                print(f"SELL ALERT: {symbol} hit trailing stop on {date.date()} at ${exit_price:.2f}")
                if SOUND_ENABLED:
                    winsound.Beep(440, 300)

                trade_log.append({
                    'symbol': symbol, 'date': date,
                    'action': 'SELL (TRAIL STOP)', 'price': exit_price,
...
# === Per-symbol Metrics ===
for symbol in symbols:
    curve = symbol_capital_curves[symbol]
    if len(curve) < 2:
        continue
    s_returns = pd.Series(curve).pct_change().dropna()
    f_cap = curve[-1]
    t_ret = (f_cap / curve[0]) - 1
